...
@section('title', 'Client - Sales Portal')
...
<div class="flex h-screen overflow-hidden">
    ...
    <div class="flex-1 min-w-0 overflow-y-auto">
        ...
        <div class="p-4 sm:p-5 lg:p-[26px] animate-fade-in space-y-6" x-data="clientManager()">
            ...
            <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
                <form method="GET" action="{{ route('clients.index') }}" class="w-full sm:max-w-md relative">
                    ...
                    <input type="text" 
                           ...
                           placeholder="Search client name, department, PIC..." 
                ...
                <div class="flex items-center justify-between sm:justify-end gap-4">
                    <div class="text-xs text-gray-400 font-medium whitespace-nowrap">
                        Total: <span class="text-gray-700 font-bold">{{ $clients->total() }}</span> Klien
                    </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b border-gray-100 bg-gray-50/50 text-[11px] font-bold tracking-wider text-gray-500 uppercase">
                                <th class="py-4 px-6">NAMA KLIEN <span class="text-gray-400">↑</span></th>
                            ...
                            @forelse($clients as $client)
                                <tr class="hover:bg-gray-50/60 transition-colors">
                                    ...
                                    <td class="py-4 px-6 text-gray-600">
                                        @if($client->pic_name)
                                            <div class="font-medium text-gray-800">{{ $client->pic_name }}</div>
                                        @endif
                                        <div class="text-xs text-gray-400">{{ $client->phone ?: ($client->email ?: '—') }}</div>
                                    </td>
                                    <td class="py-4 px-6 text-right whitespace-nowrap">
                                        <div class="flex items-center justify-end gap-1.5">
                                            <button @click="showDetails({{ $client->id }})" 
                                            ...
                                            <button @click="confirmDelete({{ $client->id }}, '{{ addslashes($client->name) }}')"
                ...
                <div x-show="isModalOpen" 
                    ...
                    <div class="bg-white rounded-2xl max-w-lg w-full p-6 shadow-[0_20px_50px_rgba(14,13,18,0.25)] space-y-5" @click.away="isModalOpen = false">
                        <div class="flex items-center justify-between border-b border-gray-100 pb-3">
                            <h3 class="text-lg font-bold text-gray-900" x-text="editMode ? 'Edit Client' : 'Add New Client'"></h3>
                            <button @click="isModalOpen = false" class="text-gray-400 hover:text-gray-600">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                            </button>
                        </div>

                        <form :action="editMode ? '/clients/' + form.id : '{{ route('clients.store') }}'" method="POST" class="space-y-4">
                            @csrf
                            ...
                            <div>
                                <label class="block text-xs font-bold text-gray-700 uppercase mb-1">Client / Company Name *</label>
                                <input type="text" name="name" x-model="form.name" required placeholder="Contoh: ACCOR GROUP (HOSPITALITY)" 
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    <label class="block text-xs font-bold text-gray-700 uppercase mb-1">Department / Division</label>
                                    <input type="text" name="department" x-model="form.department" placeholder="Contoh: IPNET#1, IT" 
                                           class="w-full px-3.5 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-gray-700 uppercase mb-1">PIC Name</label>
                                    <input type="text" name="pic_name" x-model="form.pic_name" placeholder="Nama Kontak PIC" 
                                           class="w-full px-3.5 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500">
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    <label class="block text-xs font-bold text-gray-700 uppercase mb-1">Phone / WhatsApp</label>
                                    <input type="text" name="phone" x-model="form.phone" placeholder="0812-xxxx-xxxx" 
                                           class="w-full px-3.5 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-gray-700 uppercase mb-1">Email</label>
                                    <input type="email" name="email" x-model="form.email" placeholder="pic@client.com" 
                                           class="w-full px-3.5 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500">
                                </div>
                            </div>

                            <div>
                                <label class="block text-xs font-bold text-gray-700 uppercase mb-1">Address / Location</label>
                                <textarea name="address" x-model="form.address" rows="2" placeholder="Alamat lengkap gedung/kantor..." 
                            ...
                            <div class="flex items-center justify-end gap-2 pt-3 border-t border-gray-100">
                                ...
                                <button type="submit" class="px-5 py-2.5 text-[13px] font-semibold text-white bg-[#C81E2C] hover:brightness-105 active:translate-y-[1px] rounded-lg shadow-[0_8px_20px_rgba(200,30,44,0.24)] transition-all">Simpan Client</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <template x-teleport="body">
                <div x-show="isDetailOpen" 
                    ...
                    <div class="bg-white rounded-2xl max-w-lg w-full p-6 shadow-[0_20px_50px_rgba(14,13,18,0.25)] space-y-4" @click.away="isDetailOpen = false">
                        <div class="flex items-center justify-between border-b border-gray-100 pb-3">
                            <h3 class="text-lg font-bold text-gray-900" x-text="detailClient.name"></h3>
                        ...
                        <div class="space-y-3 text-sm">
                            <div class="flex justify-between py-1.5 border-b border-gray-50">
                                ...
                                <span class="font-semibold text-gray-800" x-text="detailClient.department || 'N/A'"></span>
                            </div>
                            <div class="flex justify-between py-1.5 border-b border-gray-50">
                                <span class="text-gray-500">PIC</span>
                                <span class="font-semibold text-gray-800" x-text="detailClient.pic_name || '—'"></span>
                            </div>
                            <div class="flex justify-between py-1.5 border-b border-gray-50">
                                <span class="text-gray-500">Phone</span>
                                <span class="font-semibold text-gray-800" x-text="detailClient.phone || '—'"></span>
                            </div>
                            <div class="flex justify-between py-1.5 border-b border-gray-50">
                                <span class="text-gray-500">Email</span>
                                <span class="font-semibold text-gray-800" x-text="detailClient.email || '—'"></span>
                            </div>
                            <div class="py-1.5">
                                <span class="text-gray-500 block mb-1">Alamat</span>
                                <div class="p-2.5 rounded-xl bg-gray-50 text-gray-700 text-xs" x-text="detailClient.address || 'Belum ada alamat'"></div>
                            </div>
                        </div>

                        <div class="flex items-center justify-end gap-2 pt-3 border-t border-gray-100">
                            <button @click="editClient(detailClient)" class="px-4 py-2 text-xs font-bold text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-xl">EDIT</button>
                            <button @click="isDetailOpen = false" class="px-4 py-2 text-xs font-bold text-white bg-gray-800 hover:bg-gray-900 rounded-xl">TUTUP</button>
                        </div>
                    </div>
                </div>
            </template>
