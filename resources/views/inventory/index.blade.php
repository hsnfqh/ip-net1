...
<div class="flex h-screen overflow-hidden bg-[#F8FAFC] font-sans" x-data="inventoryManagerPage()">
                        ...
                        <tbody class="divide-y divide-[#F1F5F9] font-medium text-[#1E293B]">
                            @forelse($items as $item)
                                ...
                                <tr class="hover:bg-[#F8FAFC] transition">
                                    ...
                                    <td class="py-3.5 px-5 text-center whitespace-nowrap">
                                        <button type="button" 
                                                @click="promptDelete({{ $item->id }}, '{{ addslashes($item->product_name) }}')"
                                                title="Hapus Produk" 
    ...
    {{-- MODAL KONFIRMASI HAPUS (PLEK KETIPLEK DENGAN LEAD ENGINEER) --}}
    <template x-teleport="body">
        <div x-show="deleteConfirmOpen" 
            ...
            <div class="bg-white rounded-2xl w-[420px] max-w-full p-6 text-left shadow-[0_20px_60px_rgba(15,23,42,0.25)] animate-fade-in-up border border-[#E2E8F0]">
                ...
                <h3 class="text-center font-display text-[16px] font-bold text-[#1E293B] mb-1.5">Yakin Hapus Produk?</h3>
                <p class="text-center text-[12.5px] text-[#64748B] mb-6 break-words" x-text="'Produk &quot;' + (itemToDelete ? itemToDelete.name : '') + '&quot; akan dihapus secara permanen.'"></p>

                <form :action="`/inventory/${itemToDelete ? itemToDelete.id : ''}`" method="POST" class="flex gap-2.5">
                    @csrf
                    @method('DELETE')
                    <button type="button" @click="deleteConfirmOpen = false" class="flex-1 py-2.5 px-4 rounded-xl bg-white text-[#334155] border border-[#CBD5E1] font-bold text-[12.5px] hover:bg-[#F8FAFC] transition cursor-pointer text-center">
                        Batal
                    </button>
                    <button type="submit" class="flex-1 py-2.5 px-4 rounded-xl btn-ipnet-gradient font-bold text-[12.5px] transition cursor-pointer shadow-md text-white text-center">
                        Hapus
                    </button>
                </form>
            </div>
    ...
    function inventoryManagerPage() {
        return {
            ...
            promptDelete(id, name) {
                this.itemToDelete = { id: id, name: name };
