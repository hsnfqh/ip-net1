...
<div class="flex h-screen overflow-hidden bg-[#F8FAFC] font-sans">
    ...
    <div class="flex-1 min-w-0 overflow-y-auto">
        ...
        <div class="p-4 sm:p-6 lg:p-7 space-y-5 max-w-[1600px] mx-auto" x-data="projectsManager()">
            ...
            <div class="ipnet-card p-5 sm:p-6 anim-fade-up anim-delay-1">
                ...
                <div class="flex flex-col sm:flex-row flex-wrap items-center gap-3">
                    <div class="relative flex-1 min-w-[240px] w-full sm:w-auto">
                        <svg class="w-4 h-4 absolute left-3.5 top-3 text-[#94A3B8]" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                        </svg>
                        <input type="text" 
                               x-model="search"
                               @input="currentPage = 1"
                               placeholder="Cari nama project, client, sales..." 
                               class="w-full pl-10 pr-4 py-2 rounded-xl border border-[#CBD5E1] text-[13px] text-[#1E293B] bg-white outline-none hover:border-[#94A3B8] focus:border-[#8F0A0D] focus:ring-1 focus:ring-[#8F0A0D]/20 transition-all shadow-xs placeholder-[#94A3B8]">
                    </div>
                    ...
                    <table class="w-full min-w-[900px] border-collapse text-[13.5px]">
                        <thead>
                            <tr class="bg-[#F8FAFC] border-b border-[#E2E8F0]">
                                <th class="text-left py-3.5 px-4 text-[11.5px] font-bold text-[#475569] uppercase tracking-wider">Nama Project & Tipe</th>
                                <th class="text-left py-3.5 px-4 text-[11.5px] font-bold text-[#475569] uppercase tracking-wider">Client & Sales</th>
                                <th class="text-left py-3.5 px-4 text-[11.5px] font-bold text-[#475569] uppercase tracking-wider">Lokasi</th>
                                <th class="text-left py-3.5 px-4 text-[11.5px] font-bold text-[#475569] uppercase tracking-wider">Durasi & Deadline</th>
                                <th class="text-left py-3.5 px-4 text-[11.5px] font-bold text-[#475569] uppercase tracking-wider">Progress</th>
                                <th class="text-left py-3.5 px-4 text-[11.5px] font-bold text-[#475569] uppercase tracking-wider">Status</th>
                                <th class="text-right py-3.5 px-4 text-[11.5px] font-bold text-[#475569] uppercase tracking-wider">Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                            <template x-for="project in paginatedProjects" :key="project.id">
                                <tr class="border-t border-[#F1F5F9] hover:bg-[#F8FAFC] transition-colors duration-150">
                                    <td class="py-3.5 px-4">
                                        <div class="font-semibold text-[#1E293B]" x-text="project.name"></div>
                                        <div class="flex items-center gap-1.5 mt-1">
                                            ...
                                            <template x-if="!project.visit_schedule || project.visit_schedule === 'None' || project.visit_schedule === '-'">
                                                <span class="inline-block text-[11px] text-[#64748B]" x-text="project.project_type || 'One-Time Project'"></span>
                                            </template>
                                        </div>
                                    </td>
                                    <td class="py-3.5 px-4">
                                        <div class="font-semibold text-[#1E293B]" x-text="project.client"></div>
                                        <div class="text-[11.5px] text-[#64748B] flex items-center gap-1.5 mt-0.5">
                                            <span class="text-[#94A3B8] font-medium">Sales:</span>
                                            <span class="font-semibold text-[#334155]" x-text="project.sales_name || '-'"></span>
                                        </div>
                                    </td>
                                    <td class="py-3.5 px-4 text-[#334155]" x-text="project.location"></td>
                                    <td class="py-3.5 px-4">
                                        <div class="font-mono text-[12px] font-semibold text-[#1E293B]" x-text="formatDeadline(project.deadline)"></div>
                                    ...
                                    <td class="py-3.5 px-4">
                                        <div class="w-24">
                                            <div style="width: 100%; background: #F1F5F9; border-radius: 20px; height: 6px; overflow: hidden;">
                                                <div style="height: 100%; border-radius: 20px; background: linear-gradient(90deg, #8F0A0D, #D62E3C); transition: width .25s ease;" :style="{ width: getProjectProgress(project) + '%' }"></div>
                                            </div>
                                            <span class="text-[10.5px] text-[#94A3B8]" x-text="getProjectProgress(project) + '%'"></span>
                                        </div>
                                    </td>
                                    <td class="py-3.5 px-4">
                                    ...
                                    <td class="py-3.5 px-4">
                                        <div class="flex justify-end gap-2">
                                            <button @click="viewProject(project)" title="Lihat Detail Project" class="rounded-lg p-1.5 text-[#64748B] hover:text-[#1E293B] hover:bg-[#F1F5F9] transition-colors duration-200">
                                            ...
                                            @if($canCreate ?? false)
                                             <button @click="editProject(project)" title="Edit Project" class="rounded-lg p-1.5 text-[#64748B] hover:text-[#1E293B] hover:bg-[#F1F5F9] transition-colors duration-200">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                                </svg>
                                            </button>
                                            
                                            <button @click="confirmDelete(project)" title="Hapus Project" class="rounded-lg p-1.5 text-[#8F0A0D] hover:text-[#7A0D18] hover:bg-[#FEE2E2] transition-colors duration-200">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div x-show="modalOpen" 
                    ...
                    <div class="bg-white rounded-2xl w-[580px] max-w-full max-h-[85vh] flex flex-col overflow-hidden animate-fade-in-up shadow-[0_20px_60px_rgba(14,13,18,0.2)]">
                            ...
                            <form id="projectForm" @submit.prevent="saveProject()" class="space-y-4">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" x-model="form.sales_name" required class="wms-input" placeholder="Contoh: Donny Burnan / Erie / Hendry Wibowo / dll">
                                    </div>
                                </div>
                        ...
                        <div class="flex items-center gap-3 p-4 sm:px-6 sm:py-4 border-t border-[#E7E5E3] bg-[#FAF9F8] flex-shrink-0">
                            <button type="submit" 
                                    form="projectForm" 
                                    class="btn-ipnet-gradient flex-1 flex items-center justify-center min-h-[42px] px-4 py-2.5 rounded-xl font-bold text-[14px] cursor-pointer shadow-md">
