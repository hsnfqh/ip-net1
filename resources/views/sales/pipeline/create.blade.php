...
<style>
    ...
    .sec-card { background:#FFFFFF; border:1.5px solid #E8ECF2; border-radius:16px; padding:22px 24px; margin-bottom:18px; }
    .sec-head { font-size:12.5px; font-weight:800; color:#1E293B; margin-bottom:16px; padding-bottom:11px; border-bottom:1.5px solid #F1F5F9; display:flex; align-items:center; gap:8px; }
    ...
    .side-card { background:#FFFFFF; border:1.5px solid #E8ECF2; border-radius:14px; padding:16px 18px; margin-bottom:14px; }
    .side-head { font-size:10.5px; font-weight:800; color:#8F0A0D; text-transform:uppercase; letter-spacing:.07em; margin-bottom:12px; display:flex; align-items:center; gap:6px; }
    .side-row { display:flex; justify-content:space-between; align-items:flex-start; padding:6px 0; border-bottom:1px dashed #F1F5F9; font-size:11.5px; }
    .side-row:last-child { border-bottom:none; padding-bottom:0; }
    .side-key { color:#64748B; font-weight:600; }
    .side-val { color:#1E293B; font-weight:700; text-align:right; max-width:58%; }
    ...
    .page-footer { position:sticky; bottom:0; background:#FFFFFF; border-top:1.5px solid #E2E8F0; padding:14px 24px; display:flex; justify-content:flex-end; gap:12px; z-index:50; }
    @keyframes fadeUp { from{opacity:0;transform:translateY(10px)} to{opacity:1;transform:translateY(0)} }
    .afu { animation:fadeUp .35s cubic-bezier(.16,1,.3,1) both; }
    .afu-1 { animation-delay:.06s; } .afu-2 { animation-delay:.12s; }
...
<div class="flex h-screen overflow-hidden bg-[#F4F6F9] font-sans" x-data="createProjectPage()" x-cloak>
    ...
    <div class="flex-1 min-w-0 overflow-y-auto flex flex-col">
        ...
        <div class="px-6 pt-5 pb-1 flex items-center gap-2 text-[12.5px] text-[#64748B]">
        ...
        <div class="px-6 pt-2 pb-4">
            <h1 class="text-[22px] font-bold text-[#1E293B] tracking-tight">Tambah Project Baru</h1>
        </div>
        <div class="flex-1 px-6 pb-0">
                ...
                <div class="w-full">
                        <div class="sec-card afu">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="name" required class="cf-input" placeholder="Contoh: Pengadaan Firewall &amp; Switch Datacenter" value="{{ old('name') }}" x-model="projectName">
                            </div>
                            <div class="mb-4">
                                <label class="cf-label">Project Detail</label>
                                <textarea name="sales_notes" class="cf-textarea" placeholder="Detail kebutuhan klien, spesifikasi teknis, atau catatan penting..." x-model="projectDetail">{{ old('sales_notes') }}</textarea>
                            </div>
                            <div class="mb-4">
                                <label class="cf-label">Client Name <span class="text-[#8F0A0D]">*</span></label>
                                <input type="text" name="client" required list="clientListCreate" class="cf-input" placeholder="Contoh: PT Telkom Indonesia / Bank BRI" value="{{ old('client') }}" x-model="clientName">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="client_department" class="cf-input" placeholder="Contoh: IT Division, Network Dept..." value="{{ old('client_department') }}" x-model="clientDept">
                                </div>
                                <div>
                                    <label class="cf-label">Sales / Client PIC</label>
                                    <input type="text" name="sales_pic" class="cf-input" placeholder="Nama Sales / Account Manager" value="{{ old('sales_pic', auth()->user()->name) }}" x-model="salesPic">
                                </div>
                            </div>
                        </div>
                        <div class="sec-card afu afu-1">
                            <div class="sec-head"><span class="sec-dot" style="background:linear-gradient(135deg,#4F46E5,#7C3AED);"></span>Estimasi &amp; Pipeline</div>
                            <div class="grid grid-cols-2 gap-4 mb-4">
                                <div>
                                    <label class="cf-label">Project Estimation (Nilai Kontrak)</label>
                                    <div class="rp-wrap"><span class="rp-prefix">Rp</span><input type="number" name="contract_value" min="0" class="cf-input rp-input" placeholder="0" value="{{ old('contract_value',0) }}" x-model.number="contractValue"></div>
                                </div>
                                <div>
                            ...
                            <div class="grid grid-cols-2 gap-4 mb-4">
                                <div>
                                    ...
                                    <input type="date" name="start_date" class="cf-input" value="{{ old('start_date', date('Y-m-d')) }}" x-model="startDate">
                                </div>
                                <div>
                                    <label class="cf-label">Estimasi Closing</label>
                                    <input type="date" name="expected_closing_date" class="cf-input" value="{{ old('expected_closing_date', date('Y-m-d', strtotime('+1 month'))) }}" x-model="closingDate">
                                </div>
                            </div>
                            <div>
                        ...
                        <div class="sec-card afu afu-2">
                            <div class="sec-head" style="margin-bottom:12px;"><span class="sec-dot" style="background:linear-gradient(135deg,#0EA5E9,#0284C7);"></span><span>Milestone</span><span class="ml-auto text-[11px] font-semibold text-[#94A3B8]">Complete (<span x-text="milestones.filter(m=>m.done).length"></span>/<span x-text="milestones.length"></span>)</span></div>
                            ...
                            <button type="button" @click="openMilestoneModal()" class="flex items-center gap-1.5 text-[12.5px] font-bold text-[#8F0A0D] hover:text-[#6B0009] transition cursor-pointer mt-1">
        ...
        <div class="page-footer">
            <a href="{{ route('sales.pipeline.index') }}" class="px-5 py-2.5 rounded-xl border border-[#CBD5E1] text-[#475569] font-bold text-[13px] hover:bg-[#F8FAFC] transition cursor-pointer inline-flex items-center">Cancel</a>
            <button type="submit" form="createProjectForm" class="px-6 py-2.5 rounded-xl font-bold text-[13px] text-white cursor-pointer shadow-md hover:opacity-90 active:scale-[0.98] transition-all" style="background:linear-gradient(135deg,#8F0A0D 0%,#D62E3C 100%);">{{ $isCompleted ? 'Simpan Project Selesai' : 'Create New Project' }}</button>
        </div>
    </div>
    {{-- MILESTONE MODAL --}}
...
<script>
    function createProjectPage() {
        return {
            projectName: '', clientName: '', clientDept: '',
            salesPic: '{{ auth()->user()->name }}',
            contractValue: 0, probability: 10,
